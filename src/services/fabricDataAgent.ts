...
export class FabricDataAgentService {
  ...
  public async executeDaxQuery(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(`Fabric API error: ${response.status} - ${errorText}`);
        
        // Handle specific error codes
        if (response.status === 401) {
          throw new Error('Fabric API authentication failed');
        }
        
        if (response.status === 429) {
          throw new Error('Fabric API rate limit exceeded');
        }
        
        if (response.status >= 500) {
          throw new Error('Fabric API service unavailable');
        }
      ...
      if (error instanceof Error) {
        if (error.message.includes('Authentication failed')) {
          throw new Error('Fabric data service authentication failed. Please contact support.');
        }
        
        if (error.message.includes('rate limit')) {
          throw new Error('Too many requests to the data service. Please wait a moment and try again.');
        }
        
        if (error.message.includes('service unavailable')) {
          throw new Error('Data service is temporarily unavailable. Please try again later.');
        }
      }
  ...
  public async executeSqlQuery(
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        logger.error(`Fabric SQL API error: ${response.status} - ${errorText}`);
        
        // Handle specific error codes
        if (response.status === 401) {
          throw new Error('Fabric SQL API authentication failed');
        }
        
        if (response.status === 429) {
          throw new Error('Fabric SQL API rate limit exceeded');
        }
        
        if (response.status >= 500) {
          throw new Error('Fabric SQL API service unavailable');
        }
      ...
      if (error instanceof Error) {
        if (error.message.includes('Authentication failed')) {
          throw new Error('Fabric SQL service authentication failed. Please contact support.');
        }
        
        if (error.message.includes('rate limit')) {
          throw new Error('Too many requests to the SQL service. Please wait a moment and try again.');
        }
        
        if (error.message.includes('service unavailable')) {
          throw new Error('SQL service is temporarily unavailable. Please try again later.');
        }
        
        if (error.message.includes('timeout')) {
          throw new Error('SQL query is taking too long to execute. Please try a simpler query.');
        }
      }
