...
export class StorageService {
  ...
  /**
   * Get file stream from a specific container (for workspace files)
   * @param fileName - The name of the file blob
  ...
  public async getFileStreamFromContainer(fileName: string, containerName: string): Promise<NodeJS.ReadableStream> {
    try {
      ...
      logger.error(`File download error from container ${containerName}:`, error);
      throw new Error('Failed to download file');
    }
  }
}