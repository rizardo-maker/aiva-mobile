    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('Authentication failed')) {
          throw new Error('Data service authentication failed. Please contact support.');
        }
        
        if (error.message.includes('Fabric API error: 401')) {
          throw new Error('Data service authentication failed. Please contact support.');
        }
        
        if (error.message.includes('Fabric API error: 429')) {
          throw new Error('Data service is currently overloaded. Please try again later.');
        }
        
        if (error.message.includes('timeout')) {
          throw new Error('Data service is taking too long to respond. Please try again.');
        }
      }
