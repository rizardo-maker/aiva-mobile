  ...
  try {
    ...
    for (const userData of testUsers) {
      ...
      if (existingUser.recordset.length === 0) {
        // Create new user with predictable ID for development
        const hashedPassword = await bcrypt.hash(userData.password, 12);
        // Use predictable IDs for test users
        const userId = userData.email === 'test@example.com' ? 'user1' : 
                      userData.email === 'admin@example.com' ? 'admin1' : uuidv4();
        ...
        console.log(`✅ Created ${userData.role} user: ${userData.email} with ID: ${userId}`);
      } else {
