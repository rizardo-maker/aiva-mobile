...
async function testStorageAndExtraction() {
  try {
    console.log('Testing storage and file content extraction...');
...
This would be used in a workspace for semantic search and RAG functionality.`;
    ...
    if (fileContentResult.content.startsWith('[Content not available')) {
      console.log('❌ ERROR: File content extraction failed');
      console.log(`Error content: ${fileContentResult.content}`);
    } else {
      console.log('✅ SUCCESS: File content extraction working correctly');
